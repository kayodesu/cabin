...
{
    jobjectArray prop_array = loadArrayClass(S(array_java_lang_String))->allocArray(g_properties.size()*2);
    int i = 0;
...
static jobjectArray platformProperties()
{
    ...
    jobjectArray prop_array = loadArrayClass(S(array_java_lang_String))->allocArray(FIXED_LENGTH);
    // todo 具体赋值
