...
struct code_attribute {
     ...
     */
    struct code_attribute_exception_table *exception_tables;// [exception_table_length];

    u2 attributes_count;
    struct attribute_common **attributes; // [attributes_count];
...
struct inner_classes_attribute {
    ...
    struct inner_class *classes;  // [number_of_classes];
...
struct line_number_table_attribute  {
    ...
    struct line_number_table *line_number_tables; //[line_number_table_length];
...
//    struct parameter_annotation *parameter_annotations; // [num_parameters];
...
struct source_file_attribute {
    ...
    u2 source_file_index; // 常量池索引， 指向CONSTANT_Utf8_info常量。
};
