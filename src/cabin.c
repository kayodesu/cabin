...
void initProperties()
{
    ...
    utf8_t *class_version = vm_malloc(sizeof(utf8_t) * 32); // new utf8_t[32];
    ...
    g_properties[g_properties_count++] = (struct property) { "java.class.path", getClasspath()};
...
//    g_java_home = R"(C:\Program Files\Java\jdk-15)"; // todo for testing .................
    ...
    Class *vm = NULL;
    if (IS_JDK9_PLUS) {
        vm = load_boot_class("jdk/internal/misc/VM");
    } else {
        vm = load_boot_class("sun/misc/VM");
    }
...
int main(int argc, char* argv[])
{
    ...
    Class *main_class = loadClass(g_app_class_loader, dot_to_slash(main_class_name));
    if (main_class == NULL) {
    ...
    if (main_method == NULL) {
        ...
        if (!ACC_IS_PUBLIC(main_method->access_flags)) {
            JVM_PANIC("method main must be public."); // todo
        }
        if (!ACC_IS_STATIC(main_method->access_flags)) {
    ...
    jarray args = alloc_string_array(main_func_args_count);
    for (int i = 0; i < main_func_args_count; i++) {
        jarray_set_ref(args, i, alloc_string(main_func_args[i]));
    }
...
int main()
{
    ...
    // }
     for (int i = 0; i < g_properties_count; i++) {
        printf("%s: \"%s\"\n", g_properties[i].name, g_properties[i].value);
    }
}
#endif
...
int main(int argc, char *argv[])
{
    ...
    cout << "---------" << endl;
    cout << unparseMethodDescriptor(NULL, NULL) << endl;
}
#endif
