...
void new(struct frame *frame)
{
    ...
    const char *class_name = rtcp_get_class_name(frame->m.method->clazz->rtcp, bcr_readu2(reader)); // todo reads2  ?????
    assert(class_name != NULL);
    assert(strlen(class_name) > 0);

    struct class *c = resolve_class(frame->m.method->clazz, class_name);  // todo
    if (!c->inited) {
